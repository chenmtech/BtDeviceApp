...
public class PttDetector {
    ...
    private static final int DELTA_PTT = 0; // 计算ECG R波峰到PPG最大一阶导数之间的PTT
    private static final int FOOT_PTT = 1; // 计算ECG R波峰到PPG波谷之间的PTT
    ...
    private int qrsPos = 0; // 第二个QRS波或R波波峰位置
    private int ppg_1 = 0; // 用于中心差分求PPG一阶导数
    private int ppg_2 = 0; // 用于中心差分求PPG一阶导数

    private final QrsDetector qrsDetector; // QRS波检测器
    ...
    private int findPtt(int ecg, int ppg, int whichPtt) {
        ...
        if(delay != 0) {
            ...
            if (qrsNum == 3) {
                ...
                double maxDelta = rlt.second; // 得到最大一阶导数，用于检测fPTT

                int ptt = 0;
                // 如果是DELTA_PTT，直接将PPG一阶导数最大值位置换算为PTT
                ...
                // 否则是FOOT_PTT，继续寻找foot PPG位置，并将该位置换算为PTT
                    ...
                    int footPpg = rlt.second; // PPG最小值
                    int footPpgPos = (int)Math.round((footPpg - ppgData.get(deltaPpgPos))/maxDelta + deltaPpgPos); // 计算PPG波谷位置，算法见文献
