...
public class KMWebServiceUtil {
    ...
    public static WebResponse downloadBasicRecords(Account account, BasicRecord record, long fromTime, int num, String filterStr) {
        WebResponse resp = accountWebLogin(account);
        if(resp.getCode() != RETURN_CODE_SUCCESS) return resp;

        JSONObject json = new JSONObject();
        try {
            json.put("cmd", CMD_DOWNLOAD_BASICRECORDS);
            json.put("accountId", account.getAccountId());
